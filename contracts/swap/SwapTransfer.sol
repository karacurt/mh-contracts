...
contract SwapTransfer is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function transferNFT(uint256 _tokenId, address _recipient) external {
    require(msg.sender == trustedToken.ownerOf(_tokenId), "NOT_OWNER");
    require(_recipient != address(0), "INV_ADDRESS");
    require(_recipient != msg.sender, "CANNOT_TRANSFER_TO_SELF");

    uint256 totalFee = feeValueFor(msg.sender);
    require(paymentToken.balanceOf(msg.sender) >= totalFee, "INSUFFICIENT_FUNDS");

    if (totalFee > 0) {
      if (burnFeePercentage > 0) {
        uint256 burnFeeValue = (totalFee * burnFeePercentage) / 100 ether;

        emit FeeBurned(0, burnFeeValue);
      ...
      totalFeePaid += totalFee;
      paymentToken.transferFrom(msg.sender, treasury, totalFee);
    }

    trustedToken.safeTransferFrom(msg.sender, _recipient, _tokenId);
  }
}
