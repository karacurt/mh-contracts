  ...
  ERC721Pausable
{
  ...
  event GhostMinted(address booster, address ghostOwner, uint256 nftId);

  /// @notice Event emitted when a special NFT is minted
  event SpecialGhostMinted(string data, address ghostOwner, uint256 nftId);
   ...
   */
  function mintGhostByRarity(address to, address rarity) public onlyRole(MINTER_ROLE) {
    uint256 id = _tokenIdTracker.current();
    _doMint(to);

    emit GhostMinted(rarity, to, id);
  }

  /**
   * @notice Mints a very specific NFT defined by the data param
   * @param to The recipient's address
   * @param data Arbitrary data that defines the ghost
   */
  function mintSpecialGhost(address to, string calldata data) public onlyRole(MINTER_ROLE) {
    uint256 id = _tokenIdTracker.current();
    _doMint(to);

    emit SpecialGhostMinted(data, to, id);
  }
