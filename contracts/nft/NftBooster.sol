...
/**
 * @title Nft Booster
 * @dev This is a normal ERC721 token that integrates with our MouseHero contract for minting NFTs.
 *      It also integrates with Galler's Launchpad (https://github.com/galler-dev/launchpad) for special sales
 */
contract NftBooster is
  Context,
  ...
  ERC721Pausable
{
  ...
  /**
   * @notice Modifier to limit interactions to the Mouse Hero contract
   */
  modifier onlyMouseHero() {
    require(_msgSender() == mouseHeroAddress, "Only Mouse Hero");
    _;
  }
  ...
  constructor(
    ...
    address _mouseHeroAddress,
    address launchpad,
    uint256 maxGallerSupply
  ) ERC721(name, symbol) {
    ...
    LAUNCHPAD = launchpad;
    LAUNCH_MAX_SUPPLY = maxGallerSupply;
  }
  ...
  /**
   * @notice Burns an NFT if, and only if, called by MouseHero. This is 'minting' a mouse.
   * @dev This function only runs in the minting flow. See the MouseHero contract.
   * @param account The owner of the NFT
   * @param tokenId The NFT id
   */
  function burnFrom(address account, uint256 tokenId) public onlyMouseHero {
   ...
   * @param _mouseHeroAddress The address of the MouseHero contract
   */
  function setMouseHero(address _mouseHeroAddress) public onlyRole(OPERATOR_ROLE) {
    mouseHeroAddress = _mouseHeroAddress;
  }
