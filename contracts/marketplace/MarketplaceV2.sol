 ...
 */
contract MarketplaceV2 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  mapping(uint256 => uint256) public versionToOrderId;
  ...
  function setVersionToOrderId(uint256 _version, uint256 _threshold)
    public
    onlyRole(OPERATOR_ROLE)
  {
    versionToOrderId[_version] = _threshold;
  }
  ...
  function cancelOrder(uint256 orderId) external whenNotPaused {
    ...
    if (order.id > versionToOrderId[1]) {
