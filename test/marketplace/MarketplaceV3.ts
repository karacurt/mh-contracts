...
describe("Marketplace V3", async function () {
  ...
  beforeEach(async function () {
    ...
    Marketplace = await ethers.getContractFactory("MarketplaceV3Fix");
  ...
  async function executeRandomOrders(ordersQuantity: number, version = 1) {
    ...
    for (let i = 0; i < ordersQuantity; i++) {
      ...
      let executeOrder;
      if (version >= 3) {
        executeOrder = await marketplace.connect(buyer).executeOrder(order.orderId, false);
      } else {
        executeOrder = await marketplace.connect(buyer).executeOrder(order.orderId);
      }
  ...
  async function fruitSalad(times = 20, version = 1) {
    ...
    for (let i = 0; i < times; i++) {
      ...
      switch (op) {
        ...
        case 1:
          ...
          break;
        case 2:
          await executeRandomOrders(quantity, version);
          break;
        default:
