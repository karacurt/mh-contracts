...
describe("Mouse Haunt Booster GENESIS", function () {
  it("Should be an ERC20 token", async function () {
    const [owner] = await ethers.getSigners();
    const BMHTG = await ethers.getContractFactory("BMHTG");
    const token = await BMHTG.deploy(owner.address);
    await token.deployed();
    expect(await token.name()).to.equal("Mouse Haunt Booster GENESIS");
    expect(await token.symbol()).to.equal("BMHTG");
