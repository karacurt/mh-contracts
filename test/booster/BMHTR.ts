...
describe("Mouse Haunt Booster RARE", function () {
  it("Should be an ERC20 token", async function () {
    const [owner] = await ethers.getSigners();
    const BMHTR = await ethers.getContractFactory("BMHTR");
    const token = await BMHTR.deploy(owner.address);
    await token.deployed();
    expect(await token.name()).to.equal("Mouse Haunt Booster RARE");
    expect(await token.symbol()).to.equal("BMHTR");
    expect(await token.totalSupply()).to.equal("187500");
  });
