...
async function main() {
  ...
  await confirmOrDie(
    `Are you sure you would like to set the Stasher Trusted Tokens on ${CONFIG.name}?`
  );
  ...
  const txRegister = await contract.setTrustedToken(CONFIG.MouseHauntToken.address);
