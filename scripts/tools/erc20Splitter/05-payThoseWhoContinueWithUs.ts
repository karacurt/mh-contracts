...
import splitterTargets from "./data/toBeContinued_partial_jun22_partial_03.json";

const BN = require("bignumber.js");

// If this is true, we'll multiply amounts by 1e18
const AMOUNTS_ARE_IN_ETHER = false;
...
async function main() {
  ...
  // Log the grand total
  const grandAmounts = splitterTargets.map((item) => item.MHT_per_month);
  let grandTotal = grandAmounts.reduce((a, b) => BN(a).plus(BN(b)).toFixed());
  if (AMOUNTS_ARE_IN_ETHER) {
    splitterTargets.forEach((item) => {
      item.MHT_per_month = Web3.utils.toWei(item.MHT_per_month as string);
    });
  ...
  for (let i = START; i * STEP < splitterTargets.length; i++) {
    ...
    const amounts = targets.map((item) => item.MHT_per_month);
