...
async function main() {
  ...
  const boosterSale = await BoosterSale.deploy(
    config[network].BoosterSale.PrivateSale2.owner,
    config[network].BUSD.address,
    config[network].BMHTL.address,
    config[network].BMHTE.address,
    wei(config[network].BoosterSale.PrivateSale2.BMHTL.busdPrice),
    wei(config[network].BoosterSale.PrivateSale2.BMHTE.busdPrice)
  );
